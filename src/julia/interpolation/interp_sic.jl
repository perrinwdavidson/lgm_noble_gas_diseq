...
# interp_sic
# ---------------------------------------------------------
# purpose :: to interpolate pmip3 sic data to uvic grid
# author :: perrin w. davidson
# contact :: perrinwdavidson@gmail.com
# date :: 05.04.23
# =========================================================
function interp_sic(fname, basepath_sample, basepath_plots, basepath_krige)
	...
	for imonth in 1 : 1 : num_months
		...
		# make sample array ::
		sample = Matrix{Float64}(hcat(sample_x[idx_sample_real], sample_y[idx_sample_real], sample_f_norm_detrend)); 
		...
		cmap = plt.cm.cividis;
		# norm = colors.BoundaryNorm(-15.0 : 0.1 : 15.0, cmap.N);
		# norm_var = colors.BoundaryNorm(-1.0 : 0.1 : 1.0, cmap.N);
		im1 = ax1.scatter(sample_x[idx_sample_real], sample_y[idx_sample_real], c=sample_f[idx_sample_real], s=50);  #, cmap=cmap, norm=norm
		...
		ax1.set_xlim([0, 360]); 
		ax1.set_ylim([-90, 90]); 
		im2 = ax2.contourf(query_x_grid, query_y_grid, krige_data_grid);  # sample_f[idx_sample_real]
		...
		ax2.set_xlim([0, 360]); 
		ax2.set_ylim([-90, 90]); 
		im3 = ax3.contourf(query_x_grid, query_y_grid, krige_var_grid);  # sample_f[idx_sample_real]
