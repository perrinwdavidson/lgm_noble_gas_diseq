...
for iage in age_types
	for ivar in variable_types
		...
		if ivar == "sic"
			for ifile in 1 : 1 : length(filenames)

				# get filename ::
				fname = filenames[ifile] * ".nc";
		...
		elseif ivar == "u10"
			for fname in filenames
				if (!isnothing(findfirst("ua", fname)) || !isnothing(findfirst("va", fname))) && isnothing(findfirst("get", fname))
					...
					# interpolate ::
					eval(Meta.parse(replace("include('" * model_name * "/interp_u10_" * model_name * ".jl')", "'" => '"')));
					eval(Meta.parse("interp_u10_" * model_name * "(fname, basepath_sample, basepath_plots, basepath_interp)"));
