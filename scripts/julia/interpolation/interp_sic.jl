...
# date :: 05.04.23
# =========================================================
function interp_sic(fname, basepath_sample, basepath_plots, basepath_interp)
	...
	for imonth in 1 : 1 : num_months
		...
		ax[2].set_ylim([-90, 90]);
		plt.tight_layout(); 
		plt.savefig(joinpath(basepath_plots, model_name, model_name * "_" * var_name * "_" * string(imonth) * "_interp_data.png"), dpi=100); 
		# plt.show();
	...
	pmip3_out = NCDataset(joinpath(basepath_interp, model_name, model_name * "_" * var_name * "_interp.nc"), "c"); 
	defDim(pmip3_out, "lon", size(pmip3_data)[1]);
	defDim(pmip3_out, "lat", size(pmip3_data)[2]);
	defDim(pmip3_out, "time", size(pmip3_data)[3]);
	pmip3_out.attrib["title"] = model_name * " SIC NN Interpolated to UVic Grid"; 
